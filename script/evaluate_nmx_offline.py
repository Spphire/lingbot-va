...
from wan_va.inference_modes import resolve_video_run_mode
...
def main() -> None:
    ...
    fastwam_video_run_mode = bool(
        getattr(config, "fastwam_video_run_mode", False)
    )
    ...
    for sample_index in candidates:
        ...
        chunk_boundaries: list[int] = []
        chunk_inference_seconds: list[float] = []
        if native_first_chunk:
            torch.manual_seed(args.seed + ordinal * 100_000)
            torch.cuda.synchronize(server.device)
            ...
            torch.cuda.synchronize(server.device)
            chunk_inference_seconds.append(time.perf_counter() - infer_started)
        ...
        records.append(
            {
                ...
                "chunk_inference_seconds": chunk_inference_seconds,
                "inference_seconds_total": float(sum(chunk_inference_seconds)),
                "inference_seconds_first_chunk": float(chunk_inference_seconds[0]),
                "inference_seconds_steady_mean": (
                    float(np.mean(chunk_inference_seconds[1:]))
                    if len(chunk_inference_seconds) > 1
                    else None
                ),
    ...
    summary = {
        ...
        "fastwam_video_run_mode": fastwam_video_run_mode,
        "video_run_mode": video_run_mode,
